...
const STATUS_COLORS: Record<string, string> = {
  draft: 'var(--status-warning)', active: 'var(--status-success)', expired: 'var(--text-secondary)', terminated: 'var(--status-error)',
}
  ...
  return (
    <div style={{ padding: 'var(--space-6)', maxWidth: 1280, margin: '0 auto' }}>
      <div style={{ marginBottom: 'var(--space-6)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ fontSize: 'var(--text-2xl)', fontWeight: 700, color: 'var(--text-primary)' }}>Employee Contracts</h1>
          <p style={{ color: 'var(--text-secondary)', marginTop: 4, fontSize: 'var(--text-sm)' }}>Contract lifecycle management</p>
        </div>
      </div>

      {expiring.length > 0 && (
        <div style={{ background: 'var(--status-warning-bg)', border: '1px solid var(--status-warning)', borderRadius: 8, padding: '12px 16px', marginBottom: 'var(--space-4)', display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 'var(--space-4)' }}>
        {['all', 'draft', 'active', 'expired', 'terminated'].map(s => (
          <button key={s} onClick={() => setFilter(s)}
            style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid var(--card-border)', cursor: 'pointer', fontWeight: filter === s ? 700 : 400, background: filter === s ? 'var(--brand)' : 'transparent', color: filter === s ? '#fff' : 'var(--text-primary)', fontSize: 'var(--text-sm)' }}>
      ...
      <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 8, overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--text-sm)' }}>
          ...
          <tbody>
            {contracts.map((c, i) => (
              <tr key={c.id} style={{ borderBottom: '1px solid var(--card-border)', background: i % 2 ? 'var(--surface-subtle)' : 'transparent' }}>
                <td style={{ padding: '10px 16px', fontWeight: 500 }}>{c.employee_name}</td>
                <td style={{ padding: '10px 16px', color: 'var(--text-secondary)' }}>{c.contract_type_display}</td>
                <td style={{ padding: '10px 16px' }}>
                  <Badge style={{ background: STATUS_COLORS[c.status] || 'var(--text-secondary)', color: '#fff', fontSize: 11 }}>{c.status_display}</Badge>
                  {c.is_expiring_soon && <span style={{ marginLeft: 6, fontSize: 12, color: 'var(--status-warning)' }}>⚠️ Expiring</span>}
                </td>
                <td style={{ padding: '10px 16px', color: 'var(--text-tertiary)', whiteSpace: 'nowrap' }}>{c.start_date}</td>
                <td style={{ padding: '10px 16px', color: c.is_expiring_soon ? 'var(--status-warning)' : 'var(--text-tertiary)', whiteSpace: 'nowrap' }}>{c.end_date || 'Unlimited'}</td>
                ...
                <td style={{ padding: '10px 16px' }}>
                  ...
                  {c.status === 'active' && (
                    <HasPermission permission="hr_contracts:manage">
                      <Button size="sm" variant="ghost" style={{ color: 'var(--status-error)', marginLeft: 4 }} onClick={() => { setTerminateTarget(c); setTerminationDate(''); setTerminationReason('') }}>Terminate</Button>
                    </HasPermission>
                  )}
                </td>
              </tr>
            ))}
            {contracts.length === 0 && <tr><td colSpan={8} style={{ padding: 24, textAlign: 'center', color: 'var(--text-tertiary)' }}>No contracts found.</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {terminateTarget && (
        <BaseModal isOpen title={`Terminate Contract — ${terminateTarget.employee_name}`} onClose={() => setTerminateTarget(null)}
          footer={<Button style={{ background: 'var(--status-error)', color: '#fff' }} disabled={!terminationDate || terminateMutation.isPending}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            <div>
              ...
              <input type="date" value={terminationDate} onChange={e => setTerminationDate(e.target.value)}
                style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--card-border)', borderRadius: 6 }} />
            </div>
            <div>
