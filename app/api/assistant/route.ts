...
// ── System prompt ──────────────────────────────────────────────────────────────
function buildSystem(page: string, tenantName?: string): string {
  const companyLine = tenantName
    ? `for ${tenantName}`
    : 'for this company';
  return `You are ARIA — the intelligent AI assistant embedded inside the ERB enterprise management system ${companyLine}.
...
async function runLoop(
  ...
  const system = buildSystem(currentPage, tenantName);
...
export async function POST(req: NextRequest) {
  ...
  const { messages, authToken, currentPage, tenantName } = await req.json() as {
    messages: Message[];
    authToken: string;
    currentPage: string;
    tenantName?: string;
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        await runLoop(messages, authToken, currentPage || '/', send, tenantName);
      } catch (err: unknown) {
