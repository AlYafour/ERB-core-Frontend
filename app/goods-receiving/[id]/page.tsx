      ...
      <PageShell>
        ...
        {purchaseOrder && (
          <div className="proc-chain">
            <Link href={`/purchase-orders/${purchaseOrder.id}`} className="proc-chain-pill">
              <span className="proc-chain-pill-type">Purchase Order</span>
              <span className="proc-chain-pill-num">{purchaseOrder.order_number || `LPO-${purchaseOrder.id}`}</span>
            </Link>
            ...
            <div className="proc-chain-pill" style={{ borderColor: 'var(--brand)', background: 'var(--brand-subtle)' }}>
              <span className="proc-chain-pill-type" style={{ color: 'var(--brand)' }}>GRN</span>
              <span className="proc-chain-pill-num" style={{ color: 'var(--brand)' }}>{grn.grn_number}</span>
            </div>
          </div>
        )}
        ...
        <div className="card">
          ...
          {grn.notes && (
            <div style={{ marginTop: 'var(--space-4)', paddingTop: 'var(--space-4)', borderTop: '1px solid var(--border-subtle)' }}>
              <ProcField label={t('col', 'notes')} value={grn.notes} />
            </div>
          )}
        </div>
        ...
        {((grn.material_images && grn.material_images.length > 0) || grn.supplier_invoice_file_url) && (
          <div className="card">
            ...
            {grn.material_images && grn.material_images.length > 0 && (
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: grn.supplier_invoice_file_url ? 'var(--space-4)' : 0 }}>
                {grn.material_images.map((img: { id?: number; image_url?: string; image?: string }, i: number) => (
                  <div key={img.id || i} style={{ width: 120, height: 120, borderRadius: 8, overflow: 'hidden', cursor: 'pointer', border: '1px solid var(--border-subtle)' }}
            ...
            {grn.supplier_invoice_file_url && (
              grn.supplier_invoice_file_url.endsWith('.pdf') ? (
                ...
                <img src={grn.supplier_invoice_file_url} alt="Supplier Invoice" style={{ maxWidth: 300, maxHeight: 300, objectFit: 'contain', borderRadius: 8, cursor: 'pointer', border: '1px solid var(--border-subtle)' }}
