...
// ── PDF Document ─────────────────────────────────────────────────────
function PRDocument({ pr, company }: { pr: PurchaseRequest; company?: Partial<CompanyInfo> }) {
  ...
  return (
    <Document>
      <Page size="A4" style={S.page}>
        ...
        {/* ── HEADER */}
        <View style={S.header}>
          {co.logo ? <Image src={co.logo} style={S.logo} /> : <View style={[S.logo, { backgroundColor: '#f1f5f9' }]} />}
          <View style={S.companyBlock}>
            <Text style={S.companyName}>{co.name || 'Company Name'}</Text>
            {co.address ? <Text style={S.companySub}>{co.address}</Text> : null}
            {(co.phone || co.email) ? <Text style={S.companySub}>{[co.phone, co.email].filter(Boolean).join(' · ')}</Text> : null}
            {co.trn ? <Text style={S.companySub}>TRN: {co.trn}</Text> : null}
          </View>
          <View style={S.docBox}>
        ...
        <View style={S.footer}>
          ...
          {(co.name || co.address) ? (
            <Text style={S.footerText}>{[co.name, co.address].filter(Boolean).join('  ·  ')}</Text>
          ) : null}
        </View>
...
// ── Export function ──────────────────────────────────────────────────
export async function downloadPRPdf(pr: PurchaseRequest, company?: Partial<CompanyInfo>) {
  const blob = await pdf(<PRDocument pr={pr} company={company} />).toBlob();
