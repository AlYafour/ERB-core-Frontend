...
/* ─── Company fallback (shown when branding not yet loaded) ─────── */
...
export default function PrintTemplate({ docType, docNumber, date, status, children, footer }: PrintTemplateProps) {
  ...
  return (
    <div style={{ padding: '6mm 10mm 5mm', fontFamily: "'Inter','Cairo','Segoe UI',sans-serif", fontSize: '10pt', color: NAVY, lineHeight: 1.45, flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <tr>
            <td style={{ width: 80, verticalAlign: 'middle' }}>
              {company.logo ? (
                ...
                <div style={{ width: 72, height: 72, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: '#f1f5f9', color: GREY, fontSize: '9pt', fontWeight: 700 }}>
                  LOGO
                </div>
              )}
            </td>
            <td style={{ verticalAlign: 'middle', paddingLeft: 14 }}>
              <div style={{ fontSize: '13pt', fontWeight: 800, color: NAVY, letterSpacing: '-.3px', lineHeight: 1.2 }}>{company.name || 'Company Name'}</div>
              {company.address && <div style={{ fontSize: '8pt', color: GREY, marginTop: 2 }}>{company.address}</div>}
              {(company.phone || company.email) && (
                <div style={{ fontSize: '8pt', color: GREY, marginTop: 1 }}>
                  {[company.phone, company.email].filter(Boolean).join('  ·  ')}
                </div>
              )}
              {company.trn && <div style={{ fontSize: '8pt', color: GREY, marginTop: 1 }}>TRN: {company.trn}</div>}
            </td>
      ...
      {/* ── FOOTER ── */}
      {footer ?? <DefaultFooter name={company.name} address={company.address} />}
    </div>
  );
}
...
/* ─── Default footer ────────────────────────────────────────────── */
function DefaultFooter({ name, address }: { name?: string; address?: string }) {
  const tag = [name, address].filter(Boolean).join('  ·  ');
  return (
