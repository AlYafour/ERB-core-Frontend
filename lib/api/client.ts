...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      // Only attempt refresh for requests that carried a Bearer token.
      // Login/register calls have no Authorization header — their 401 means
      // wrong credentials, so let the mutation's onError handle it.
