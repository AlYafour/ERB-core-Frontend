...
export const purchaseRequestsApi = {
  ...
  updateItem: async (itemId: number, data: { product_id?: number; quantity?: number; unit?: string; project_site?: string; reason?: string; notes?: string }): Promise<PurchaseRequestItem> => {
  ...
  recall: async (id: number): Promise<PurchaseRequest> => {
    const response = await apiClient.post(`/purchase-requests/${id}/recall/`);
    return response.data;
  },

  revise: async (id: number): Promise<PurchaseRequest> => {
    const response = await apiClient.post(`/purchase-requests/${id}/revise/`);
    return response.data;
  },
